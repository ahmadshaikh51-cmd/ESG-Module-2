...
- **Security Group (SG)**: A virtual firewall. You'll need one for the ALB (allow port 80/443 from anywhere) and one for your ECS containers (only allow traffic coming _from_ the ALB).
...
_(Note: Adjust the `CMD` to match your production start command, e.g., `npm run start` or `node server.js`)_
...
_Skip this if `ecsTaskExecutionRole` already exists in your AWS account._
  ...
  "containerDefinitions": [
    {
      ...
      "environment": [{ "name": "VITE_API_BASE_URL", "value": "https://api.transvolt.org" }],
      "logConfiguration": {
        "logDriver": "awslogs",
        "options": {
...
_It is highly recommended to do this step in the AWS Console UI because linking VPCs and Subnets via CLI is prone to errors._
