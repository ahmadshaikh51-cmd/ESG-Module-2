...
function DrawerContent({ trip, onClose }: { trip: Trip; onClose: () => void }) {
  ...
  const points = Array.from({ length: 24 }, (_, i) => {
    ...
    return {
      t: i,
      soc:
        trip.battery_pack_state_of_charge_start -
        (trip.battery_pack_state_of_charge_start - trip.battery_pack_state_of_charge_end) * t,
      ...
      <div className="space-y-5 p-5">
        <div className="grid grid-cols-2 gap-2.5">
          ...
          <Stat
            icon={Battery}
            label="SOC"
            value={`${trip.battery_pack_state_of_charge_start.toFixed(0)} → ${trip.battery_pack_state_of_charge_end.toFixed(0)}%`}
          />
          <Stat
            icon={Timer}
            label="Idle"
            value={`${Math.round(trip.idle_time_sec / 60)}m · ${trip.idle_energy_share_pct.toFixed(1)}%`}
          />
        </div>
        ...
        <Section title="Thermal & voltage">
          <div className="grid grid-cols-2 gap-2.5">
            <Stat
              icon={ThermometerSun}
              label="Max cell temp"
              value={`${trip.max_cell_temp.toFixed(1)}°C`}
              accent={trip.high_temp_flag ? "destructive" : undefined}
            />
            <Stat
              icon={Zap}
              label="V (A / B)"
              value={`${trip.avg_a_voltage_v.toFixed(0)} / ${trip.avg_b_voltage_v.toFixed(0)}`}
              accent={trip.voltage_instability_flag ? "warning" : undefined}
            />
          </div>
        </Section>
        ...
        <Section title="Diagnostic flags">
          <div className="flex flex-wrap gap-1.5">
            {flags.map((f) => (
              ...
              >
                <span
                  className={`h-1.5 w-1.5 rounded-full ${f.ok ? "bg-success" : "bg-destructive"}`}
                />
...
function ChartBox({ children }: { children: React.ReactNode }) {
  return (
    <div className="rounded-lg border border-border/50 bg-card/40 p-2 h-32 flex flex-col">
      {children}
    </div>
  );
}
    ...
    <div className="mt-1 grid grid-cols-2 gap-1 text-[10.5px]">
      {rows.map((r) => (
        <div key={r.name} className="flex items-center gap-1">
          ...
          <span className="ml-auto num text-foreground">
