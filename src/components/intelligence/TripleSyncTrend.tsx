...
export function TripleSyncTrend({ depots }: { depots: DepotEnergyDaily[] }) {
  ...
  const data = useMemo(() => {
    ...
    return slice.map((d) => ({
      ...
      grid: d.grid_intake_kwh,
      charger: d.charger_output_kwh,
      bus: d.bus_demand_kwh,
    }));
  }, [window, daily, hourly]);
  ...
  return (
    <Panel>
      ...
      <div className="flex items-start gap-2 border-t border-border/40 bg-muted/15 px-4 py-3 text-[12px] text-muted-foreground">
        ...
        <span>
          <strong className="text-foreground">{interpretation.headline}</strong> —{" "}
...
function interpret(data: { grid: number; charger: number; bus: number }[]) {
  ...
  if (cap > 0.12 && inflow < 0.08) {
    return {
      headline: "Charger bottleneck detected",
      detail: `Charger output capped while grid intake holds — ${(cap * 100).toFixed(0)}% delivery gap suggests saturation.`,
    };
  }
  if (cap < 0.04 && inflow < 0.05) {
    return {
      headline: "Stable charging ecosystem",
      detail:
        "Grid, charger, and bus curves are aligned — infrastructure delivering within expected envelope.",
    };
  }
  ...
  if (Math.sqrt(variance) / mean > 0.08) {
    return {
      headline: "Bus-side charging instability",
      detail:
        "Grid and charger output stable while bus demand oscillates — likely BMS variability across pack states.",
    };
  }
  return {
    headline: "Infrastructure stress likely upstream",
    detail: `Grid fluctuations propagating to chargers — investigate transformer health.`,
  };
}
