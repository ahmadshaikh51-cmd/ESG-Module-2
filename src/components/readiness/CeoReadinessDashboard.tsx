...
export function CeoReadinessDashboard() {
  ...
  const overdueCount =
    pendingQueue?.filter((p) => p.deadline && daysUntil(p.deadline) < 0).length || 0;
  const due7Count =
    pendingQueue?.filter(
      (p) => p.deadline && daysUntil(p.deadline) >= 0 && daysUntil(p.deadline) <= 7,
    ).length || 0;
  ...
  const handleSaveCell = (val: EditCellValue) => {
    ...
    updateReadiness(
      {
        id: editing.id,
        data: { status: backendStatus, deadline: val.deadline, owner: val.owner, notes: val.notes },
      },
      {
        onSuccess: () => {
    ...
    <div className="readiness-executive mx-auto max-w-[1800px] space-y-6">
      <header className="flex flex-wrap items-start justify-between gap-4 border-b border-border/40 pb-6">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-[13px] text-muted-foreground">
            Global status across {sitesList.length} sites. Click any item in site cards to edit
            status and deadline.
          </p>
        </div>
      ...
      <section className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">
        <SummaryKpi
          label="Fleet ready"
          value={`${fleetPct}%`}
          sub={`${stats.cells_ready} / ${stats.applicable_cells} items`}
          tone="primary"
        />
        <SummaryKpi
          label="Pending"
          value={String(stats.applicable_cells - stats.cells_ready)}
        ...
        <SummaryKpi
          ...
          value={worstSite ? `${Math.round(worstSite.readinessPct * 100)}%` : "—"}
          sub={worstSite?.site ?? "—"}
          tone="muted"
        />
      </section>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4">
            {(() => {
              const filtered = siteCards
                ...
                .filter(
                  (s) => !siteSearch || s.site.toLowerCase().includes(siteSearch.toLowerCase()),
                );
              ...
              return filtered.map((site) => (
                <SiteCard
                  ...
                  site={site}
                  onEdit={openEdit}
                  onViewDetails={() => setSelectedSiteForDetails(site)}
                />
              ));
            })()}
          </div>
            ...
            {filteredPending.length === 0 ? (
              ...
              filteredPending.map((task) => (
                <PendingRow
                  ...
                  onEdit={() =>
                    openEdit({
                      id: task.readiness_id,
                      itemName: task.checklist_name,
                      siteName: task.site_name,
                      siteId: task.site_id,
                      value: { status: "no", deadline: task.deadline },
                    })
                  }
                />
              ))
                ...
                <button
                  ...
                  onClick={() =>
                    onEdit({
                      ...
                      siteName: site.site,
                      siteId: site.siteId,
                      value: { status: "yes", deadline: d.deadline },
                    })
                  }
      ...
      >
        ...
        <div className="mt-1.5 flex flex-wrap items-center justify-between gap-2">
          <span className="flex items-center gap-1 text-[10px] text-muted-foreground">
            ...
            {task.deadline ? formatDeadlineLabel(task.deadline) : "No date"}
          </span>
          {task.deadline && (
            <span
              className={`rounded-md px-1.5 py-0.5 text-[10px] font-semibold ring-1 ${toneClass}`}
            >
              {badge.label}
            </span>
          )}
...
function PendingRow({ task, onEdit }: { task: any; onEdit: () => void }) {
  ...
  return (
    <button
      type="button"
      onClick={onEdit}
      className="flex w-full flex-wrap items-center gap-4 pl-5 pr-8 py-3 text-left transition hover:bg-muted/20"
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-0.5 text-[11px] text-muted-foreground">
          {task.team} · {task.owner || "Unassigned"} · {task.priority}
        </div>
      </div>
      <div className="text-right">
        ...
        {badge && (
          <span
            className={`mt-1 inline-block rounded-md px-2 py-0.5 text-[10px] font-semibold ring-1 ${toneClass}`}
          >
            {badge.label}
          </span>
        )}
...
function ViewToggle({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`inline-flex items-center gap-1.5 rounded-md px-3 py-1.5 text-[12px] font-medium transition-colors ${active ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"}`}
    >
