...
export function SiteReadinessDashboard() {
  ...
  const weekly = useMemo(() => {
    ...
    if (projectFilter === "all") {
      return [...snapshots]
        ...
        .map((s) => ({
          week: new Date(s.snapshot_date).toLocaleDateString(undefined, {
            day: "numeric",
            month: "short",
          }),
  ...
  const filtered = useMemo(() => {
    return tableRows.filter((r) => {
      ...
      if (query && !`${r.item} ${r.team} ${r.owner}`.toLowerCase().includes(query.toLowerCase()))
        return false;
      if (siteFilter !== "all") {
    ...
    <div className="space-y-8">
        ...
        <div className="relative flex flex-wrap items-start justify-between gap-6">
          <div className="max-w-2xl">
            ...
            <p className="mt-2 text-[13px] text-muted-foreground">
              Centralised, live web view of the IT &amp; ITMS readiness matrix across{" "}
              {siteList.length} sites and {tableRows.length} workstreams.
            </p>
          </div>
      ...
      <section className="grid grid-cols-2 gap-3 md:grid-cols-4">
        <KpiCard
          icon={CheckCircle2}
          label="Completed"
          value={stats.workstream_classification.completed}
          tone="success"
        />
        <KpiCard
          icon={Activity}
          label="On Track"
          value={stats.workstream_classification.on_track}
          tone="primary"
        />
        <KpiCard
          icon={AlertTriangle}
          label="At Risk"
          value={stats.workstream_classification.at_risk}
          tone="warning"
        />
        <KpiCard
          icon={Clock}
          label="Delayed"
          value={stats.workstream_classification.delayed}
          tone="destructive"
        />
      </section>
            ...
            <ResponsiveContainer>
              <PieChart>
                <Pie
                  ...
                  stroke="var(--card)"
                >
                  {stats.readiness_by_type.map((_, i) => (
            ...
            <ResponsiveContainer>
              <PieChart>
                <Pie
                  ...
                  stroke="var(--card)"
                >
                  {stats.spend_mix.map((_, i) => (
                    <RCell key={i} fill={CHART_COLORS[(i + 2) % CHART_COLORS.length]} />
                  ))}
                </Pie>
                <Tooltip
                  contentStyle={{
          ...
          <div className="space-y-2 px-3 pb-4 pt-4 h-[300px] overflow-auto">
            ...
            {deadlines.map((d) => {
              ...
              return (
                <div
                  key={d.readiness_id}
                  className="flex items-center justify-between gap-3 rounded-xl border border-border/40 bg-background/40 px-3 py-2.5 transition-colors hover:border-primary/30"
                >
                  ...
                  <div
                    className={`shrink-0 rounded-md px-2 py-1 text-[10.5px] font-semibold ring-1 ${
                      overdue
      ...
      <Card>
        ...
        <div className="h-[280px] px-3 pb-3 pt-4">
          <ResponsiveContainer>
            <BarChart
              data={stats.readiness_across_sites.map((s) => ({
                site: s.name,
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border/40 px-5 py-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <select
              ...
              className="h-8 rounded-lg border border-border/60 bg-background/60 px-2 text-[12px] outline-none"
            >
          ...
          <table className="w-full min-w-[1100px] text-[12px]">
            <thead>
              <tr className="border-b border-border/40 text-left text-[10.5px] uppercase tracking-wider text-muted-foreground">
                ...
                {siteList.map((s) => (
                  <th key={s} className="px-1.5 py-2.5 text-center">
                    {s}
                  </th>
                ))}
              </tr>
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-2 py-2">
                      <span
                        className="inline-flex items-center gap-1.5 rounded-md px-2 py-0.5 text-[10.5px] font-semibold ring-1"
                        style={{
                          color: PRIO_COLORS[r.priority] || "var(--muted)",
                          borderColor: "transparent",
                          background: `color-mix(in oklab, ${PRIO_COLORS[r.priority] || "var(--muted)"} 12%, transparent)`,
                        }}
                      >
                        {r.priority}
                      </span>
                    ...
                    <td className="px-2 py-2">
                      <span
                        ...
                        style={{
                          ...
                          background: `color-mix(in oklab, ${STATUS_COLORS[r.status] || "var(--muted)"} 14%, transparent)`,
                          borderColor: "transparent",
                        }}
                      >
                        <span
                    ...
                    {siteList.map((s) => {
                      ...
                      if (!cell)
                        return <td key={s} className="px-1.5 py-2 text-center align-middle" />;
                      return (
...
function EditableCellBadge({ state }: { state: any }) {
  ...
  const base =
    "inline-flex h-6 min-w-[2rem] items-center justify-center rounded-md px-1.5 text-[10.5px] font-semibold ring-1 transition";
  if (v === "yes")
    return <span className={`${base} bg-success/15 text-success ring-success/25`}>YES</span>;
  if (v === "no")
    return (
      <span className={`${base} bg-destructive/12 text-destructive ring-destructive/25`}>NO</span>
    );
...
function Stat({ label, value, accent }: { label: string; value: string; accent?: "success" }) {
  return (
    <div className="flex items-center justify-between gap-6">
      ...
      <span className={`font-mono font-semibold ${accent === "success" ? "text-success" : ""}`}>
        {value}
      </span>
    </div>
  );
  ...
  return (
    <Card className="px-4 py-3.5">
      <div className="flex items-center gap-3">
        <div
          className={`flex h-9 w-9 items-center justify-center rounded-lg ring-1 ${tones[tone]}`}
        >
          <Icon className="h-4 w-4" />
        </div>
        <div>
