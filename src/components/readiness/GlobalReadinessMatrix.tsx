...
function CellBadge({ v }: { v: "yes" | "no" | "na" }) {
  ...
  return (
    <span className="inline-flex h-5 min-w-[2rem] items-center justify-center rounded bg-muted/50 px-1 text-[9px] font-bold text-muted-foreground">
      N/A
    </span>
  );
}
    ...
    <div className="overflow-hidden rounded-xl border border-border/50 bg-card/40">
      <div className="border-b border-border/40 px-4 py-3">
        ...
        <p className="mt-0.5 text-[12px] text-muted-foreground">
          Checklist items in rows · each site has <strong>Status</strong> and{" "}
                      ...
                      <Fragment key={`${row.id}-${site}`}>
                        ...
                        >
                          {cell.status === "yes" ? (
                            ...
                            >
                              <div className="whitespace-nowrap tabular-nums text-muted-foreground/60">
                            ...
                            >
                              ...
                              {badge && (
                                <div
                                  className={`text-[9px] ${badge.tone === "overdue" ? "text-destructive" : badge.tone === "soon" ? "text-warning" : "text-muted-foreground"}`}
                                >
                                  {badge.label}
                                </div>
                              )}
