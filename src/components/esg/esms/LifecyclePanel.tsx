  ...
  return (
    <div 
      ...
      className={cn(
        ...
        variant === "primary" && "bg-primary text-primary-foreground border-primary rounded-full py-2.5",
        variant === "active" && "border-primary/60 shadow-[0_0_0_2px_rgba(var(--primary),0.2)] bg-card",
      ...
      {subtitle && <span className="text-[10.5px] text-muted-foreground mt-0.5 leading-tight">{subtitle}</span>}
    </div>
  );
}
...
function BranchAction({ title, color, onClick }: { title: string; color: "green" | "orange" | "red"; onClick?: () => void; }) {
  const colorClasses = {
      ...
      {activeLifecycle && (
        <div className="flex flex-wrap items-center gap-2 rounded-xl border border-border/60 bg-card/60 px-4 py-2.5 text-[12px]">
          ...
          <span className="text-muted-foreground">
            ...
            <span className="font-semibold">{lifecycleStageByKey(activeLifecycle.currentStage)?.label || activeLifecycle.currentStage}</span> since{" "}
