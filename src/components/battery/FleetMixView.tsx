...
export function FleetMixView({
  ...
  const donut: DonutSeg[] = stats.map((s) => ({
    label: s.key,
    value: s.buses,
    color: depotColor(s.key, types),
  }));
    ...
    <div className="chart-enter space-y-4">
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_1.4fr]">
                ...
                >
                  <span className="flex items-center gap-2 text-[12.5px] font-medium">
                    <span
                      ...
                      style={{ background: depotColor(s.key, types) }}
                    />
                    {s.key}
                  </span>
                  <span className="text-[11.5px] text-muted-foreground">
                    <span className="num font-semibold text-foreground">{s.buses}</span> ·{" "}
                    {Math.round((s.buses / totalBuses) * 100)}%
                  </span>
                </button>
              ))}
            </div>
        ...
        <Panel
          ...
          subtitle="Battery cycle load (EFC gross) per type"
        >
          <VBars
            max={maxEfc}
            groups={stats.map((s) => ({
              ...
              bars: [
                {
                  value: s.efcG,
                  color: depotColor(s.key, types),
                  tip: `${s.key}: ${s.efcG.toFixed(2)} EFC`,
                },
              ],
            }))}
          />
      ...
      >
        ...
        {listRows.length > 60 && (
          <div className="px-1 pt-2 text-[11px] text-muted-foreground">
            Showing top 60 by EFC. Refine with the company / type filters.
          </div>
        )}
      </Panel>
    </div>
