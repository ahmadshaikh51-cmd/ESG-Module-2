...
function BatteryDashboard({ dataset, degraded }: { dataset: BatteryDataset; degraded?: boolean }) {
  ...
  const kpis = useMemo(
    () =>
      KPI_DEFS.map((d) => {
        ...
        if (curV != null && prevV != null && prevV !== 0)
          deltaPct = ((curV - prevV) / Math.abs(prevV)) * 100;
  ...
  const gauges = useMemo(() => {
    ...
    return defs.map((d) => {
      ...
      return {
        ...
        deltaText:
          delta == null ? "— no prior" : `${delta >= 0 ? "+" : ""}${delta.toFixed(1)} pts vs prev`,
        deltaColor:
          delta == null
    ...
    return scopedDepots.map((dep) => {
      ...
      return {
        ...
        bars: [
          {
            label: "Round-trip eff.",
            val: `${(s?.rte ?? 0).toFixed(1)}%`,
            w: Math.min(100, s?.rte ?? 0),
            color: "var(--primary)",
          },
          {
            label: "Energy regen",
            val: `${(s?.regen ?? 0).toFixed(1)}%`,
            w: Math.min(100, ((s?.regen ?? 0) / 25) * 100),
            color: "var(--chart-2)",
          },
          {
            label: "Aux idle",
            val: `${(s?.idle ?? 0).toFixed(1)}%`,
            w: Math.min(100, ((s?.idle ?? 0) / 40) * 100),
            color: "var(--warning)",
          },
        ],
        healthy: h[0],
  ...
  const efcBars = useMemo(() => {
    let mx = 0;
    for (const dep of scopedDepots)
      for (const k of wk)
        if (hasData(dataset, k))
          mx = Math.max(mx, dataset.summary[dep]?.[dataset.mname[k]]?.efcG ?? 0);
    if (!mx) mx = 1;
    return scopedDepots.map((dep) => ({
      depot: dep,
      months: wk.map((k) => {
        if (!hasData(dataset, k))
          return {
            ...
            val: "—",
            h: 100,
            valColor: "var(--muted-foreground)",
            fill: "repeating-linear-gradient(135deg,color-mix(in oklab,var(--muted-foreground) 13%,transparent) 0 5px,transparent 5px 11px)",
          };
  ...
  const pickBus = (bus: BusRow, monthName: string) =>
    setSelected((cur) =>
      cur?.bus.reg === bus.reg && cur.monthName === monthName ? null : { bus, monthName },
    );
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <h1 className="mt-1.5 text-[27px] font-semibold tracking-tight">
            How hard the batteries are working
          </h1>
          <p className="mt-1.5 max-w-xl text-[13.5px] leading-relaxed text-muted-foreground">
            {company === "ALL"
              ? `${dataset.depots.length} companies · fleet-weighted`
              : `${company} company`}{" "}
            · {dataset.mshort[dataset.dataKeys[0]]}–
            {dataset.mshort[dataset.dataKeys[dataset.dataKeys.length - 1]]} 2026.
          </p>
        </div>
        <div className="flex flex-col items-end gap-2.5">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-1 rounded-xl border border-border/60 bg-muted/40 p-[3px]">
              <button
                type="button"
                aria-label="Earlier 3 months"
                onClick={() => shift(-1)}
                disabled={!canPrev}
                className="flex h-8 w-[30px] items-center justify-center rounded-lg text-foreground transition disabled:opacity-30"
              >
              ...
              {wk.map((k) => {
                ...
                return (
                  <button
                    key={k}
                    type="button"
                    onClick={() => hd && setSelKey(k)}
                    disabled={!hd}
                    className={cn(
                      "flex min-w-[50px] flex-col items-center rounded-lg px-3 py-1 text-[12.5px] font-semibold leading-tight transition",
                      active ? "nav-pill-active text-foreground" : "text-muted-foreground",
                      !hd && "opacity-50",
                    )}
                  >
                    {dataset.mshort[k]}
                    <span
                      className="mt-px text-[8.5px] font-medium uppercase tracking-wide"
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              type="button"
              onClick={onExport}
              className="inline-flex h-9 items-center gap-1.5 rounded-xl border border-primary/40 bg-primary/10 px-3 text-[12.5px] font-semibold text-primary transition-colors hover:bg-primary/15"
            >
      ...
      {/* ===== SHARED DRILL DRAWER ===== */}
      <Dialog
        open={!!(selected && liveDrill && drawerMonthKey)}
      ...
      >
        <DialogContent className="max-w-5xl p-0 overflow-hidden border-none bg-transparent shadow-none [&>button]:hidden">
          {selected && liveDrill && drawerMonthKey && (
            <BusDrillDrawer
              reg={selected.bus.reg}
              type={selected.bus.type}
              monthName={selected.monthName}
              reportMonth={drawerMonthKey}
              summary={selected.bus}
        ...
        <>
            ...
            <div className="chart-enter grid gap-4 lg:grid-cols-[1.55fr_1fr]">
                    ...
                    <div key={g.key} className="flex flex-col items-center text-center">
                      <div className="relative h-32 w-32">
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center">
                          <span className="num text-[25px] font-semibold leading-none">
                            {g.value}
                          </span>
              ...
              <div className="grid grid-rows-3 gap-4">
                {kpis.slice(3).map((k) => (
                  <div
                    key={k.key}
                    className="card-interactive accent-bar-top relative flex items-center gap-4 overflow-hidden rounded-2xl border border-border/60 bg-card p-4 shadow-elevated"
                    style={{ ["--accent-color" as string]: k.color }}
                  >
                    <div
                      className="flex h-[38px] w-[38px] flex-none items-center justify-center rounded-xl ring-1"
                      style={{
                        color: k.color,
                        background: `color-mix(in oklab,${k.color} 13%,transparent)`,
                        boxShadow: `inset 0 0 0 1px color-mix(in oklab,${k.color} 25%,transparent)`,
                      }}
                    >
          ...
          {layout === "grid" && (
            <div className="chart-enter grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {kpis.map((k) => (
                <div
                  key={k.key}
                  className="card-interactive accent-bar-top relative overflow-hidden rounded-2xl border border-border/60 bg-card p-5 shadow-elevated"
                  style={{ ["--accent-color" as string]: k.color }}
                >
          ...
          {layout === "compact" && (
            <div className="chart-enter overflow-hidden rounded-2xl border border-border/60 bg-card shadow-elevated">
              ...
              {kpis.map((k) => (
                <div
                  key={k.key}
                  className="grid grid-cols-[1.4fr_repeat(3,1fr)_1.1fr_0.9fr] items-center border-b border-border/50 px-5 py-3"
                >
                  ...
                  <div className="flex justify-end">
                    <Sparkline vals={k.vals} color={k.color} w={90} h={26} lineOnly />
                  </div>
                  <div
          ...
          >
            {depotCards.map((d) => (
              <button
                ...
                type="button"
                onClick={d.onSelect}
                className="card-interactive rounded-2xl border bg-card p-5 text-left shadow-elevated transition"
                style={{ borderColor: d.border }}
              >
                <div className="mb-4 flex items-center justify-between">
                  <div className="flex items-center gap-2.5">
                    <span className="h-2.5 w-2.5 rounded-sm" style={{ background: d.color }} />
                    <span className="text-[15px] font-semibold tracking-tight">{d.depot}</span>
                  </div>
                  <div className="text-[11px] text-muted-foreground">
                    <span className="num font-semibold text-foreground">{d.buses}</span> buses
                  </div>
                </div>
                <div className="mb-4 flex items-end gap-1.5">
                  ...
                  <span className="ml-auto text-[11px] text-muted-foreground">
                    <span className="num text-foreground">{d.gross}</span> kWh avg
                  </span>
                </div>
                  ...
                  {d.bars.map((bar) => (
                    <div key={bar.label}>
                      <div className="mb-1 flex justify-between text-[11px]">
                        ...
                        <span className="num font-semibold">{bar.val}</span>
                      </div>
                      <div
                ...
                <div className="mt-4 border-t border-border/60 pt-3">
                  <div className="mb-2 flex justify-between text-[10.5px] uppercase tracking-wide text-muted-foreground">
                    <span>Health bands</span>
                    <span className="num normal-case tracking-normal">
                      {d.healthyPctLabel} healthy
                    </span>
                  </div>
                  <div className="flex h-2 gap-[1.5px] overflow-hidden rounded-md">
                    ...
                    <div
                      style={{ width: `${d.attentionPct}%`, background: "var(--destructive)" }}
                    />
                  </div>
            ...
            <div className="mb-5 flex items-end justify-between">
              <div>
                ...
                <div className="mt-1 text-[16px] font-semibold tracking-tight">
                  Equivalent full cycles (gross) by company · {windowEfcLabel}
                </div>
              </div>
                ...
                {scopedDepots.map((dep) => (
                  ...
                  >
                    <span
                      ...
                      style={{ background: depotColor(dep, dataset.depots) }}
                    />
                    {dep}
                  </span>
                ))}
              ...
              <div className="flex h-44 items-end gap-2 overflow-x-auto pl-1">
                {efcBars.map((grp) => (
                  ...
                  >
                        ...
                        <Tooltip key={i}>
                          <TooltipTrigger asChild>
                            <div className="flex h-full max-w-[58px] flex-1 flex-col items-center justify-end cursor-pointer group/bar">
                              <span
                                className="num mb-1.5 text-[11.5px] font-semibold transition-opacity group-hover/bar:text-foreground"
                                style={{ color: m.valColor }}
                              >
                                {m.val}
                              </span>
                              <div
                                className="w-full rounded-t-md transition-all group-hover/bar:brightness-110 group-hover/bar:scale-x-[1.05]"
                                style={{ height: `${m.h}%`, minHeight: 3, background: m.fill }}
                              />
                              <span className="mt-1.5 text-[10.5px] text-muted-foreground group-hover/bar:text-foreground">
                                {m.label}
                              </span>
                            </div>
                          </TooltipTrigger>
                          <TooltipContent>
                            <div className="text-[11px] p-0.5">
                              ...
                              <p className="text-muted-foreground mt-0.5">
                                {m.label} 2026:{" "}
                                <strong className="text-foreground num">{m.val} EFC</strong>
                              </p>
                            </div>
                          </TooltipContent>
                        </Tooltip>
                    ...
                    <div className="mt-2.5 border-t border-border/60 pt-2.5 text-center text-[12.5px] font-semibold">
                      {grp.depot}
                    </div>
                  </div>
                ))}
          ...
          <div className="flex flex-wrap items-end justify-between gap-4 pt-2">
            <div>
              ...
              <h2 className="mt-1 text-[20px] font-semibold tracking-tight">
                Per-bus discharge cycle detail
              </h2>
            </div>
            <div className="relative">
              ...
              <input
                ...
                className="h-9 w-[230px] rounded-xl border border-border bg-card pl-8 pr-3 text-[13px] outline-none focus:border-primary/40"
              />
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-4 rounded-2xl border border-border/60 bg-muted/40 px-4 py-3">
            ...
            <div className="ml-auto flex items-center gap-3.5 text-[12px] text-muted-foreground">
              <span>
                <span className="num font-semibold text-foreground">{drillRows.length}</span> of{" "}
                {drillAll.length} buses
              </span>
              <span>
                avg EFC <span className="num font-semibold text-foreground">{drillAvgEfc}</span>
              </span>
              <button
                type="button"
                onClick={() => {
          ...
          <div className="overflow-hidden rounded-2xl border border-border/60 bg-card shadow-elevated">
                    ...
                    {DRILL_COLS.map((c) => {
                      ...
                      return (
                        <th
                          ...
                          className="th-sort sticky top-0 cursor-pointer whitespace-nowrap border-b border-border px-3.5 py-2.5 text-[10px] font-semibold uppercase tracking-wide"
                          style={{
                            textAlign: c.align,
                            color: activeCol ? "var(--primary)" : "var(--muted-foreground)",
                          }}
                        >
                          {c.label}
                ...
                <tbody>
                  {drillRows.map((r) => {
                    ...
                    return (
                      <tr
                        key={r.reg}
                        onClick={() => liveDrill && pickBus(r, explorerMonth)}
                        className={cn(
                          "border-b border-border/40",
                          r.band === "ATTENTION" && "cc-row-alert",
                          liveDrill && "cursor-pointer hover:bg-muted/40",
                          isSel && "bg-primary/5",
                        )}
                      >
                        <td className="whitespace-nowrap px-3.5 py-2.5">
                          <div className="flex items-center gap-2.5">
                            <span
                              ...
                              style={{ background: bc }}
                            />
                            <span className="num font-semibold">{r.reg}</span>
                          </div>
                        </td>
                        ...
                        <td className="num px-3.5 py-2.5 text-right font-semibold">
                          {fmt(r.efcGross, 1)}
                        </td>
                        <td className="num px-3.5 py-2.5 text-right text-muted-foreground">
                          {fmt(r.efcNet, 1)}
                        </td>
                          ...
                          <div className="flex items-center justify-end gap-2.5">
                            ...
                            >
                              <div
                                className="h-full rounded-full"
                                style={{
                                  width: `${r.healthScore == null ? 0 : Math.max(5, Math.min(100, r.healthScore))}%`,
                                  background: bc,
                                }}
                              />
                            </div>
                            <span className="num w-[22px] text-right font-semibold">
                              {fmt(r.healthScore, 0)}
                            </span>
                          </div>
                        </td>
                        <td className="px-3.5 py-2.5 text-center">
                          <span
                            className="inline-block rounded-md px-2.5 py-[3px] text-[10.5px] font-semibold tracking-wide"
                            style={{
                              color: bc,
            ...
            {liveDrill && drillRows.length > 0 && (
              <div className="border-t border-border/50 px-4 py-2 text-[11px] text-muted-foreground">
                Tip: click a bus to see its daily trend and trip segments.
              </div>
            )}
          </div>
        </>
      ...
      {view === "mix" && (
        <FleetMixView
          ...
          onSelectBus={(b) => pickBus(b, selMonthName)}
        />
      )}
      ...
      {/* ===== TRENDS ===== */}
      {view === "trends" && (
        <TrendsView dataset={dataset} company={company} monthName={selMonthName} />
      )}
    </div>
  );
}
...
function CompanyScopeBar({
  ...
  company: "ALL" | string;
  month: string;
  onChange: (c: "ALL" | string) => void;
}) {
  const totalBuses = dataset.depots.reduce(
    (a, d) => a + (dataset.summary[d]?.[month]?.buses ?? 0),
    0,
  );
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 rounded-2xl border border-border/60 bg-card p-2.5 shadow-elevated">
      ...
      >
        All companies
        <span className="num rounded-md bg-muted px-1.5 py-px text-[10.5px] font-semibold text-muted-foreground">
          {totalBuses}
        </span>
      </button>
...
function BusDrillDrawer({
  ...
  const daily = useQuery({
    queryKey: ["cycle_daily", reg, reportMonth],
    queryFn: () => fetchCycleDaily(reg, reportMonth),
    staleTime: 5 * 60_000,
  });
  const trips = useQuery({
    queryKey: ["cycle_trip", reg, reportMonth],
    queryFn: () => fetchCycleTrips(reg, reportMonth),
    staleTime: 5 * 60_000,
  });
  ...
  return (
    <div
      className="chart-enter overflow-hidden rounded-2xl border bg-card shadow-elevated"
    ...
    >
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span
                className="rounded-md px-2 py-[2px] text-[10px] font-semibold tracking-wide"
                style={{ color: bc, background: `color-mix(in oklab,${bc} 13%,transparent)` }}
              >
                {summary.band}
              </span>
            </div>
            <div className="text-[11px] text-muted-foreground">
              {type} · {monthName} 2026 · health{" "}
              <span className="num">{fmt(summary.healthScore, 0)}</span>
            </div>
          </div>
        </div>
        <button
      ...
      <div className="grid gap-5 p-5 lg:grid-cols-[1.3fr_1fr]">
        <div>
          ...
          {daily.isLoading ? (
            <DrawerLoading label="Loading daily series…" />
          ) : daily.isError ? (
            <DrawerError />
          ) : (daily.data?.length ?? 0) === 0 ? (
            <DrawerEmpty label="No daily rows for this bus/month." />
          ) : (
            <DailyChart rows={daily.data ?? []} />
          )}
        </div>
        ...
        <div className="min-w-0">
          <div className="mb-3 flex items-center justify-between">
            ...
            <div className="text-[11px] text-muted-foreground">
              <span className="num font-semibold text-foreground">{trips.data?.length ?? 0}</span>{" "}
              trips
            </div>
          </div>
          {trips.isLoading ? (
            <DrawerLoading label="Loading trips…" />
          ) : trips.isError ? (
            <DrawerError />
          ) : (trips.data?.length ?? 0) === 0 ? (
            <DrawerEmpty label="No trip segments for this bus/month." />
          ) : (
                ...
                <tbody>
                  {(trips.data ?? []).map((t, i) => {
                    ...
                    return (
                      <tr
                        key={t.segment_id ?? i}
                        className={cn("border-b border-border/40", warn && "cc-row-alert")}
                      >
                        ...
                        <td className="num px-2.5 py-1.5 text-right font-semibold">
                          {fmt(t.gross_discharge_kwh, 1)}
                        </td>
                        <td className="num px-2.5 py-1.5 text-right">{fmt(t.regen_pct, 1)}</td>
                        <td className="num px-2.5 py-1.5 text-right text-muted-foreground">
                          {fmt(t.soc_max, 0)}→{fmt(t.soc_min, 0)}
                        </td>
...
function DailyChart({ rows }: { rows: CycleDailyRow[] }) {
  ...
  const leftPercent =
    hoveredIdx !== null ? ((padL + slot * hoveredIdx + slot / 2) / Math.max(W, n * 16)) * 100 : 0;

  return (
    <div className="relative overflow-x-auto rounded-xl border border-border/60 bg-muted/20 p-3">
      <svg
        width={Math.max(W, n * 16)}
        height={H}
        viewBox={`0 0 ${Math.max(W, n * 16)} ${H}`}
        className="block"
      >
        {[0.25, 0.5, 0.75].map((g) => (
          <line
        ...
        {rows.map((r, i) => {
          ...
          return (
            <rect
              ...
              rx={2}
              fill={
                isHovered ? "var(--chart-4)" : "color-mix(in oklab,var(--chart-4) 78%,transparent)"
              }
              pointerEvents="none"
            />
          );
        ...
        >
          ...
          <div className="flex items-center gap-1.5 text-muted-foreground">
            ...
            <span>
              ...
              <strong className="text-foreground num">
                {fmt(hoveredRow.gross_discharge_kwh, 1)}
              </strong>{" "}
              kWh
            </span>
          </div>
          {hoveredRow.daily_rte_pct != null && (
            <div className="flex items-center gap-1.5 text-muted-foreground mt-0.5">
              ...
              <span>
                RTE:{" "}
                <strong className="text-foreground num">{fmt(hoveredRow.daily_rte_pct, 1)}%</strong>
              </span>
            </div>
          )}
        </div>
...
function DrawerLoading({ label }: { label: string }) {
  return (
    <div className="flex h-[170px] items-center justify-center gap-2 rounded-xl border border-border/60 bg-muted/20 text-[12px] text-muted-foreground">
...
function DrawerError() {
  return (
    <div className="flex h-[170px] items-center justify-center rounded-xl border border-destructive/30 bg-destructive/5 text-[12px] text-destructive">
      Failed to load.
    </div>
  );
}
function DrawerEmpty({ label }: { label: string }) {
  return (
    <div className="flex h-[170px] items-center justify-center rounded-xl border border-border/60 bg-muted/20 text-[12px] text-muted-foreground">
      {label}
    </div>
  );
}
...
function SourceBadge({ source, degraded }: { source: "live" | "sample"; degraded?: boolean }) {
  ...
  return (
    <span
      className="inline-flex items-center gap-1 rounded-md px-1.5 py-[1px] text-[9.5px] font-semibold tracking-wide"
      style={{
...
function DeltaPill({
  ...
  k: { deltaText: string; good: boolean | null };
  suffix?: string;
}) {
  const color =
    k.good == null ? "var(--muted-foreground)" : k.good ? "var(--success)" : "var(--destructive)";
  return (
    <div
      className="num inline-flex items-center gap-1 rounded-lg px-2 py-1 text-[11.5px] font-semibold"
...
function FilterGroup({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="flex items-center gap-2">
      ...
      <div className="flex gap-0.5 rounded-xl border border-border/60 bg-card p-[3px]">
        {children}
      </div>
    </div>
  );
