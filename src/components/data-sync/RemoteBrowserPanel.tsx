...
export function RemoteBrowserPanel({
  ...
  const dbSync = useMutation({
    ...
    onSuccess: (res, tables) => {
      ...
      toast.success(
        `Started background sync of ${n} from ${database}.${res.job_id ? ` Job ${res.job_id.slice(0, 8)}…` : ""}`,
      );
      setSelected(new Set());
      afterSync();
    },
        ...
        <>
          <div className="flex flex-wrap items-center gap-2 border-b border-border/60 px-4 py-3">
            ...
            <Button
              size="sm"
              variant="secondary"
              disabled={selected.size === 0 || dbSync.isPending}
            ...
            <Button
              size="sm"
              disabled={!database || dbSync.isPending}
...
function SyncTableDialog({
  ...
  const mut = useMutation({
    ...
    onSuccess: (res) => {
      ...
      toast.success(
        `Synced “${res.target_table ?? table}”${physical ? "" : " (reference)"}${rows}.`,
      );
      onClose();
      onDone();
    },
        ...
        <div className="space-y-4">
          <div className="flex items-start justify-between gap-4 rounded-lg border border-border/60 p-3">
            <div>
              <div className="text-[13px] font-medium">
                Physical load — materialize data locally
              </div>
          ...
          <div className="space-y-1.5">
            ...
            <Input
              ...
              placeholder={table ?? "target_table"}
